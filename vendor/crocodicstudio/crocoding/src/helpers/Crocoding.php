...
namespace crocodicstudio\crocoding\helpers;
...
class Crocoding
{
    public static function routeController($prefix, $controller,$route, $namespace = null)
    {
        $prefix = trim($prefix, '/') . '/';
...
//                Storage::makeDirectory($path);
    ...
    {
        return DB::table(config('crocoding.USER_TABLE'))->where('id', getSession('admin_id'))->first();
    }


    public static function isSuperadmin()
    {
        return getSession('admin_is_superadmin');
    }

    public static function myName()
    {
        return getSession('admin_name');
    }

    public static function myPhoto()
    {
        return getSession('admin_photo');
    }

    public static function myPrivilege()
    {
        $roles = getSession('admin_privileges_roles');
        if ($roles) {
            foreach ($roles as $role) {
                if ($role->path == Crocoding::getModulePath()) {
                    return $role;
                }
            }
        }
    ...
    {
        return getSession('admin_privileges');
    }

    public static function myPrivilegeName()
    {
        return getSession('admin_privileges_name');
    }

    public static function isLocked()
    {
        return getSession('admin_lock');
    }
...
//        dd($message,session()->get('message'));
...
//        exit;
    ...
    public static function isView()
    {
        ...
        $session = getSession('admin_privileges_roles');
    ...
    {
        if (g('d') != null) {
            setSession('currentDashboardId', g('d'));
            setSession('currentMenuId', 0);

            return g('d');
        } else {
            return getSession('currentDashboardId');
        }
    }
    ...
    {
        if (g('m') != null) {
            setSession('currentMenuId', g('m'));
            setSession('currentDashboardId', 0);

            return g('m');
        } else {
            return getSession('currentMenuId');
        }
    }
    ...
    {
        $adminPathSegments = count(explode('/', config('crocoding.ADMIN_PATH')));

        return getSegment(1 + $adminPathSegments);
    }
    ...
    public static function mainpath($path = null)
    {
        ...
        $controllername = str_replace(["\crocodicstudio\crocoding\controllers\\", "App\Http\Controllers\\"], "", strtok(currentRouteAction(), '@'));
    ...
    {
        $id = getSession('current_row_id');
        $id = intval($id);
        $id = (! $id) ? getSegment(4) : $id;
        $id = intval($id);
    ...
    public static function getCurrentMethod()
    {
        ...
        $action = str_replace("App\Http\Controllers", "", currentRouteAction());
    ...
    {
        $filter = g('filter_column');
        if (isset($filter[$field])) {
            return (isset($filter[$field]['value']))?$filter[$field]['value']:null;
        }
    }
    ...
    public static function getSortingFilter($field)
    {
        $filter = g('filter_column');
        if (isset($filter[$field])) {
            return (isset($filter[$field]['sorting']))?$filter[$field]['sorting']:null;
        }
    }
    ...
    public static function getTypeFilter($field)
    {
        $filter = g('filter_column');
        if (isset($filter[$field])) {
            return (isset($filter[$field]['type']))?$filter[$field]['type']:null;
        }
    }
    ...
    public static function sendEmailQueue($queue)
    {
        ...
        \Mail::send("crocoding::emails.blank", ['content' => $html], function ($message) use (
            $html,
            $to,
            $subject,
            $from_email,
    ...
    public static function sendEmail($config = [])
    {
        ...
        $template = Crocoding::first('cms_email_templates', ['slug' => $template]);
        ...
        if ($config['send_at'] != null) {
            ...
            $a['email_from_email'] = $template->from_email ?: Crocoding::getSetting('email_sender');
            $a['email_from_name'] = $template->from_name ?: Crocoding::getSetting('appname');
        ...
        Mail::send("crocoding::emails.blank", ['content' => $html], function ($message) use ($to, $subject, $template, $attachments) {
            $message->priority(1);
            $message->to($to);

            if ($template->from_email) {
                $from_name = ($template->from_name) ?: Crocoding::getSetting('appname');
...
//                $res->send();
    ...
    public static function parseSqlTable($table)
    {
        ...
        if (count($f) == 1) {
            return ["table" => $f[0], "database" => config('crocoding.MAIN_DB_DATABASE')];
        } elseif (count($f) == 2) {
    ...
    public static function findPrimaryKey($table)
    {
        ...
        $table = Crocoding::parseSqlTable($table);
        ...
        if ($primary_key) {
            self::putCache('table_'.implode(',',$table), 'primary_key', $primary_key);
    ...
    {
        $key = Crocoding::findPrimaryKey($table);
    ...
    {
        $parent_table = Crocoding::parseSqlTable($parent_table)['table'];
        $child_table = Crocoding::parseSqlTable($child_table)['table'];
...
//        exit;
    ...
    {
        return requestServer('HTTP_REFERER');
    }

    public static function listTables()
    {
        $tables = [];
        $multiple_db = config('crocoding.MULTIPLE_DATABASE_MODULE');
        $multiple_db = ($multiple_db) ? $multiple_db : [];
        $db_database = config('crocoding.MAIN_DB_DATABASE');

        if ($multiple_db) {
            try {
                $multiple_db[] = config('crocoding.MAIN_DB_DATABASE');
    ...
    public static function authAPI()
    {
        ...
        $allowedUserAgent = config('crocoding.API_USER_AGENT_ALLOWED');
        $user_agent = requestHeader('User-Agent');
        $time = requestHeader('X-Authorization-Time');
        ...
        if (self::getSetting('api_debug_mode') == 'false') {
            ...
            $validator = Validator::make([
                ...
                'X-Authorization-Token' => requestHeader('X-Authorization-Token'),
                'X-Authorization-Time' => requestHeader('X-Authorization-Time'),
            ...
            $sender_token = requestHeader('X-Authorization-Token');
    ...
    public static function sendNotification($config = [])
    {
        ...
        $id_cms_users = ($id_cms_users) ?: [Crocoding::myId()];
    ...
    public static function sendFCM($regID = [], $data)
    {
        ...
        $apikey = Crocoding::getSetting('google_fcm_key');
    ...
    {
        $name_col_candidate = config('crocoding.NAME_FIELDS_CANDIDATE');
    ...
    public static function generateAPI($controller_name, $table_name, $permalink, $method_type = 'post')
    {
		...
		class Api'.$controller_name.'Controller extends \crocodicstudio\crocoding\controllers\ApiController {
    ...
    public static function generateController($table, $name = null)
    {
        ...
        $image_candidate = explode(',', config('crocoding.IMAGE_FIELDS_CANDIDATE'));
        $password_candidate = explode(',', config('crocoding.PASSWORD_FIELDS_CANDIDATE'));
        $phone_candidate = explode(',', config('crocoding.PHONE_FIELDS_CANDIDATE'));
        $email_candidate = explode(',', config('crocoding.EMAIL_FIELDS_CANDIDATE'));
        $name_candidate = explode(',', config('crocoding.NAME_FIELDS_CANDIDATE'));
        $url_candidate = explode(',', config("crocoding.URL_FIELDS_CANDIDATE"));
        ...
        $coloms = Crocoding::getTableColumns($table);
        $name_col = Crocoding::getNameTable($coloms);
        $pk = Crocoding::pk($table);
...
<?php namespace App\Http\Controllers\Admin;
	...
	class Admin'.$controllername.' extends \crocodicstudio\crocoding\controllers\CBController {
	...
	';
        ...
        foreach ($coloms_col as $c) {
            ...
            if (substr($field, 0, 3) == 'id_') {
                ...
                $joincols = Crocoding::getTableColumns($jointable);
                $joinname = Crocoding::getNameTable($joincols);
        ...
        foreach ($coloms as $c) {
            ...
            $typedata = Crocoding::getFieldType($table, $field);
            ...
            if (in_array($field, $image_candidate)) {
                $type = 'upload';
                $attribute['help'] = 'Tipe file yang didukung: JPG, JPEG, PNG, GIF, BMP';
