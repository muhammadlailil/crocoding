                                ...
                                <div class="panel-body child-form-area">
                                    @foreach($form['columns'] as $col)
                                            ...
                                            @if($col['type']!='hidden')
                                                <label class="control-label col-sm-2">{{$col['label']}}
                                                    @if(!empty($col['required'])) <span class="text-danger"
                                                                                        title="This field is required">*</span> @endif
                                                </label>
                                            @endif
                                            <div class="col-sm-10">
                                                @if($col['type']=='text')
								        ...
								        <button class="btn btn-primary" onclick="showModal{{$name_column}}()" type="button"><i
                                                    class='fa fa-search'></i> Browse Data</button>
								      </span>
                                                    </div><!-- /input-group -->

                                                    @push('bottom')
                                                        <script type="text/javascript">
                                                            var url_{{$name_column}} = "{{\crocodicstudio\crocoding\helpers\Crocoding::mainpath('modal-data')}}?table={{$col['datamodal_table']}}&columns=id,{{$col['datamodal_columns']}}&name_column={{$name_column}}&where={{urlencode($col['datamodal_where'])}}&select_to={{ urlencode($col['datamodal_select_to']) }}&columns_name_alias={{urlencode($col['datamodal_columns_alias'])}}";
                                                    ...
                                                    @push('bottom')
                                                        <script type="text/javascript">
                                                            ...
                                                            function uploadFiles{{$name_column}}(event) {
                                                                ...
                                                                $.ajax({
                                                                    url: '{{\crocodicstudio\crocoding\helpers\Crocoding::mainpath("upload-file")}}',
                                                                    type: 'POST',
                                                                    data: data,
                                                                    cache: false,
                                                    ...
                                                    >
                                                        ...
                                                        if ($col['datatable']) {
                                                            ...
                                                            if (! $col['datatable_where']) {
                                                                $data = \crocodicstudio\crocoding\helpers\Crocoding::get($tableJoin, NULL, "$titleField ASC");
                                                            } else {
                                                                $data = \crocodicstudio\crocoding\helpers\Crocoding::get($tableJoin, $col['datatable_where'], "$titleField ASC");
                                                            }
                                                            foreach ($data as $d) {
