<div class='form-group peta {{$header_group_class}} {{ (isset($errors))?($errors->first($name))?"has-error":"":'' }}'>
    <label>{{$form['label']}}
        @if($required)
    ...
    <div class="input-group">
        ...
        <span class="input-group-btn">
						        <button class="btn btn-primary btnShowModalMaps" onclick="showMapModal{{$name}}()" type="button"><i
                                        class='fa fa-map-marker'></i> Browse Map</button>
						      </span>
    </div><!-- /input-group -->


    <div id='googlemaps-modal-{{$name}}' class="modal" style="min-width: 900px">
        <div class="headerModal">
            <h5 class="modal-title"><i class='fa fa-search'></i> Browse Map</h5>
        </div>
        <div class="bodyModal">

            <input id="input-search-autocomplete-{{$name}}" class="controls pac-input" autofocus type="text"
                   placeholder="Search location here...">
            <div id="type-selector-{{$name}}" class="controls hide type-selector" style="display: none">
...
@push('bottom')
    <script type="text/javascript">
        ...
        function setIt{{$name}}() {
            ...
            $('#googlemaps-modal-{{$name}} .close-modal ').click()
        }

        var is_init_map_{{$name}} = false;

        function showMapModal{{$name}}() {
            var api_key = "{{\crocodicstudio\crocoding\helpers\Crocoding::getSetting('google_api_key')}}";
