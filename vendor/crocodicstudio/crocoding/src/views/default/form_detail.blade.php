...
?>
@if(file_exists(base_path('/vendor/crocodicstudio/crocoding/src/views/default/type_components/'.$type.'/asset.blade.php')))
    @include('crocoding::default.type_components.'.$type.'.asset')
@elseif(file_exists(resource_path('views/vendor/crocoding/type_components/'.$type.'/asset.blade.php')))
    @include('vendor.crocoding.type_components.'.$type.'.asset')
@endif
<?php
$asset_already[] = $type;
} //end forms
...
<table id="table-detail" class="table">
    ...
    $file_location = base_path('vendor/crocodicstudio/crocoding/src/views/default/type_components/'.$type.'/component_detail.blade.php');
    $user_location = resource_path('views/vendor/crocoding/type_components/'.$type.'/component_detail.blade.php');
    ...
    @if(file_exists($file_location))
        ...
        @if($containTR)
            @include('crocoding::default.type_components.'.$type.'.component_detail')
        @else
            <tr>
                <td>{{$form['label']}}</td>
                <td>@include('crocoding::default.type_components.'.$type.'.component_detail')</td>
            </tr>
        @endif
    @elseif(file_exists($user_location))
        <?php $containTR = (substr(trim(file_get_contents($user_location)), 0, 4) == '<tr>') ? TRUE : FALSE;?>
        @if($containTR)
            @include('vendor.crocoding.type_components.'.$type.'.component_detail')
        @else
            <tr>
                <td>{{$form['label']}}</td>
                <td>@include('vendor.crocoding.type_components.'.$type.'.component_detail')</td>
            </tr>
        @endif
    @else
...
@push('bottom')
    <script>
        $('.cardBodyForm').addClass('p-0 text-center table-responsive detailPage');
        $('.cardBodyForm').removeClass('cardBodyForm row')
    </script>
@endpush
