...
@section('content')
    ...
    <div class="col-sm-12">
        <div class="row">
                ...
                <div class="card card-small mb-4">
                    ...
                    <div class="card-body">
                                ...
                                <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                    <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                                        <i class='{{($menu->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$menu->icon}}'></i> {{$menu->name}} <span
                                            class='pull-right'><a class='fa fa-pencil-alt' title='Edit'
                                                                  href='{{route("MenusControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(requestFullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{\crocodicstudio\crocoding\helpers\Crocoding::deleteConfirm(route("MenusControllerGetDelete",["id"=>$menu->id]))}}'
                                            ...
                                            @foreach($menu->children as $child)
                                                ...
                                                <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                    <div class='{{$child->is_dashboard?"is-dashboard":""}}'
                                                        ...
                                                        <span class='pull-right'><a class='fa fa-pencil-alt' title='Edit'
                                                                                    href='{{route("MenusControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(requestFullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                title="Delete" class='fa fa-trash'
                                                                onclick='{{\crocodicstudio\crocoding\helpers\Crocoding::deleteConfirm(route("MenusControllerGetDelete",["id"=>$child->id]))}}'
                                                                href='javascript:void(0)'></a></span>
                                                        <br/><em class="text-muted">
                                                            <small><i class="fa fa-users"></i> &nbsp; {{implode(', ',$privileges)}}</small>
                                                        </em>
                        ...
                        @if(count($menu_active)==0)
                            <div align="center" >
                                <small>Active menu is empty, please add new menu</small>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <ul class='draggable-menu draggable-menu-inactive'>
                            @foreach($menu_inactive as $menu)
                                <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                    <div><i class='{{$menu->icon}}'></i> {{$menu->name}} <span class='pull-right'><a class='fa fa-pencil-alt' title='Edit'
                                                                                                                     href='{{route("MenusControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(requestFullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{\crocodicstudio\crocoding\helpers\Crocoding::deleteConfirm(route("MenusControllerGetDelete",["id"=>$menu->id]))}}'
                                    ...
                                    <ul>
                                        @if($menu->children)
                                            @foreach($menu->children as $child)
                                                <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                    <div><i class='{{$child->icon}}'></i> {{$child->name}} <span class='pull-right'><a class='fa fa-pencil-alt'
                                                                                                                                       title='Edit'
                                                                                                                                       href='{{route("MenusControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(requestFullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                title="Delete" class='fa fa-trash'
                                                                onclick='{{\crocodicstudio\crocoding\helpers\Crocoding::deleteConfirm(route("MenusControllerGetDelete",["id"=>$child->id]))}}'
                        ...
                        @if(count($menu_inactive)==0)
                            <div align="center" id='inactive_text' class='text-muted'>
                                <small>Inactive menu is empty</small>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="col-sm-7">
                <div class="card card-small mb-4">
                    <form class='form-horizontal' method='post' id="form" enctype="multipart/form-data" action='{{\crocodicstudio\crocoding\helpers\Crocoding::mainpath("add-save")}}'>
                        <div class="card-header border-bottom">
                            Add Menu
                        </div>
                        <div class="card-body">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type='hidden' name='return_url' value='{{requestFullUrl()}}'/>
                            @include("crocoding::default.form_body")
                        </div>
                        <div class="card-footer">
                            <button name="submit" type="submit" class="btn btn-primary" value="Add Menu">
                                Add Menu
                            </button>
                        </div>
                    </form>
