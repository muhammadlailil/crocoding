...
class ApiCustomController extends CBController
{
    ...
    function getIndex()
    {
        ...
        if (! Crocoding::isSuperadmin()) {
            Crocoding::insertLog('Try view the dataAPI Index at API');
        ...
        return view('crocoding::api_documentation', $data);
    }
    ...
    function apiDocumentation()
    {
        ...
        return view('crocoding::api_documentation_public', $data);
    }
    ...
    function getDownloadPostman()
    {
        ...
        $data['variables'] = [];
        $data['info'] = [
            'name' => Crocoding::getSetting('appname').' - API',
        ...
        return response($json, 200, [
            ...
            'Content-Disposition' => 'attachment; filename='.Crocoding::getSetting('appname').' - API For POSTMAN.json',
        ]);
    }
    ...
    public function getScreetKey()
    {
        ...
        return view('crocoding::api_key', $data);
    }
    ...
    public function getGenerator()
    {
        ...
        if (! Crocoding::isSuperadmin()) {
            Crocoding::insertLog('Try view the data API Index at API');
            return Crocoding::redirect(Crocoding::adminPath(), 'Sorry you do not have privilege to access this area !');
        }
        ...
        return view('crocoding::api_generator', $data);
    }
    ...
    {
        Crocoding::valid(['id', 'status'], 'view');

        $id = g('id');
        $status = (g('status') == 1) ? "active" : "non active";
    ...
    public function getColumnTable($table, $type = 'list')
    {
        ...
        $cols = Crocoding::getTableColumns($table);
        ...
        foreach ($result as $ro) {
            ...
            $type_field = Crocoding::getFieldType($table, $ro);

            $type_field = (array_search($ro, explode(',', config('crocoding.EMAIL_FIELDS_CANDIDATE'))) !== false) ? "email" : $type_field;
            $type_field = (array_search($ro, explode(',', config('crocoding.IMAGE_FIELDS_CANDIDATE'))) !== false) ? "image" : $type_field;
            $type_field = (array_search($ro, explode(',', config('crocoding.PASSWORD_FIELDS_CANDIDATE'))) !== false) ? "password" : $type_field;
                ...
                if (substr($ro, 0, 3) == 'id_') {
                    ...
                    foreach ($t2 as $t) {
                        if ($t != 'id' && $t != 'created_at' && $t != 'updated_at' && $t != 'deleted_at') {
                            ...
                            $type_field = Crocoding::getFieldType($table2, $t);
    ...
    function postSaveApiCustom()
    {
        ...
        if (g('id')) {
            ...
            Crocoding::generateAPI($controllerName, $a['tabel'], $a['permalink'], $a['method_type']);

            DB::table('cms_apicustom')->insert($a);
        }

        return redirect(Crocoding::mainpath())->with(['message' => 'Yeay, your api has been saved successfully !', 'message_type' => 'success']);
    }
