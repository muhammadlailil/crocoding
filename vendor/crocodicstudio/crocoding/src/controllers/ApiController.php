...
class ApiController extends Controller
{
    ...
    public function execute_api()
    {
        ...
        */
        if ($parameters) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'exists') {
                    ...
                    $table_exist = Crocoding::parseSqlTable($table_exist)['table'];
                ...
                if ($name == 'id') {
                    $table_exist = Crocoding::parseSqlTable($table)['table'];
                    $table_exist_pk = Crocoding::pk($table_exist);
        ...
        $uploads_format_candidate = explode(',', config("crocoding.UPLOAD_TYPES"));
        $uploads_candidate = explode(',', config('crocoding.IMAGE_FIELDS_CANDIDATE'));
        ...
        if ($action_type == 'list' || $action_type == 'detail' || $action_type == 'delete') {
            ...
            foreach ($responses as $resp) {
                ...
                if ($used == 0 && ! Crocoding::isForeignKey($name)) {
                    continue;
                }
                ...
                $name_tmp[] = $name;
                if (Crocoding::isForeignKey($name)) {
                    $jointable = Crocoding::getTableForeignKey($name);
                    $jointable_field = Crocoding::getTableColumns($jointable);
                    $jointablePK = Crocoding::pk($jointable);
            ...
            if (Crocoding::isColumnExists($table, 'deleted_at')) {
            ...
            $data->where(function ($w) use ($parameters, $posts, $table, $type_except) {
                foreach ($parameters as $param) {
                    ...
                    if ($required == '1') {
                        if (Crocoding::isColumnExists($table, $name)) {
            ...
            if ($action_type == 'list') {
                ...
                if ($rows) {
                    ...
                    if (Crocoding::getSetting('api_debug_mode') == 'true') {
            ...
            foreach ($input_validator as $k => $v) {
                if (Crocoding::isColumnExists($table, $k)) {
                    $row_assign[$k] = $v;
                }
            }
            ...
            if ($action_type == 'save_add') {
                if (Crocoding::isColumnExists($table, 'created_at')) {
                    $row_assign['created_at'] = date('Y-m-d H:i:s');
                }
            }

            if ($action_type == 'save_edit') {
                if (Crocoding::isColumnExists($table, 'updated_at')) {
            ...
            foreach ($parameters as $param) {
                ...
                if ($type == 'file' || $type == 'image') {
                    $row_assign[$name] = Crocoding::uploadFile($name, true);
                } elseif ($type == 'base64_file') {
                    $row_assign[$name] = Crocoding::uploadBase64($value);
            ...
            if ($action_type == 'save_add') {
                ...
                $row_assign['id'] = Crocoding::newId($table);
            ...
            foreach ($parameters as $param) {
                ...
                $type = $param['type'];
                if ($type == 'ref') {
                    if (Crocoding::isColumnExists($config, 'id_'.$table)) {
                        DB::table($config)->where($name, $value)->update(['id_'.$table =>  $lastId]);
                    } elseif (Crocoding::isColumnExists($config, $table.'_id')) {
        ...
        unset($result['api_authorization']);
