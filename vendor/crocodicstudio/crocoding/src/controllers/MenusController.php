...
class MenusController extends CBController
{
    public function cbInit()
    {
        ...
        if ($row->type == 'Module') {
            $m = Crocoding::first('cms_moduls', ['path' => $row->path]);
            $id_module = $m->id;
        } elseif ($row->type == 'Statistic') {
            $row->path = str_replace('statistic_builder/show/', '', $row->path);
            $m = Crocoding::first('cms_statistics', ['slug' => $row->path]);
            $id_statistic = $m->id;
        }
    ...
    public function getIndex()
    {
        ...
        $id_cms_privileges = Request::get('id_cms_privileges');
        $id_cms_privileges = ($id_cms_privileges) ?: Crocoding::myPrivilegeId();
        ...
        return view('crocoding::menus_management', compact('menu_active', 'menu_inactive', 'privileges', 'id_cms_privileges', 'return_url', 'page_title'));
    }

    public function hook_before_add(&$postdata)
    {
        if (! $postdata['id_cms_privileges']) {
            $postdata['id_cms_privileges'] = Crocoding::myPrivilegeId();
        }
        $postdata['parent_id'] = 0;

        if ($postdata['type'] == 'Statistic') {
            $stat = Crocoding::first('cms_statistics', ['id' => $postdata['statistic_slug']]);
            $postdata['path'] = 'statistic_builder/show/'.$stat->slug;
        } elseif ($postdata['type'] == 'Module') {
            $stat = Crocoding::first('cms_moduls', ['id' => $postdata['module_slug']]);
        ...
        if ($postdata['is_dashboard'] == 1) {
            ...
            Cache::forget('sidebarDashboard'.Crocoding::myPrivilegeId());
        }
    }
